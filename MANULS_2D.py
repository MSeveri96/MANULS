...
# direction
    ...
    T[0,0]=np.matmul(tx,tx)
    T[0,1]=np.matmul(tx,ty)
    T[0,2]=np.matmul(tx,tz)
    T[1,0]=np.matmul(ty,tx)
    T[1,1]=np.matmul(ty,ty)
    T[1,2]=np.matmul(ty,tz)
    T[2,0]=np.matmul(tz,tx)
    T[2,1]=np.matmul(tz,ty)
    T[2,2]=np.matmul(tz,tz)
    
# All the ingredients are ready. The code computes the f function

    tmp_eq1=np.dot(e,D)
    tmp_eq_3_1=np.matmul(e,T)
